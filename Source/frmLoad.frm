...
Object = "{C7083F68-7BF5-4755-9CF0-38D810EC405C}#2.6#0"; "trainlib.ocx"
Begin VB.Form frmLoad 
   ...
   Begin VB.Menu mEdit 
      ...
      End
      Begin VB.Menu mLQ 
         Caption         =   "View Low Quality Textures"
         Shortcut        =   ^L
      End
      Begin VB.Menu mBB 
...
Dim booRenderDone As Boolean
Dim basemx As D3DMATRIX
Dim bbMesh As D3DXMesh
Dim bbMat As D3DMATERIAL8
Dim bbMx As D3DMATRIX
...
Private Sub cmdAce_Click()
    ...
    If sfCanvas1.isWindowed Then
        ...
        ' fix no dialog if last does not exist
        If Dir(cdlg.filename) = "" Then cdlg.filename = "*.s"
    ...
    With sfCanvas1
        ...
        If .BeginScene Then
            ...
            If mAnim.Checked Then
                modl.showModel lstLod.ListIndex + 1, (Timer * 400 Mod (iAnim * 100)) / iAnim
            Else
            ...
            ' bounding box
            If Not bbMesh Is Nothing Then
                .d3dd.SetRenderState D3DRS_ALPHABLENDENABLE, 1
                .d3dd.SetRenderState D3DRS_ALPHAREF, 1
                .d3dd.SetTexture 0, Nothing
                .d3dd.SetMaterial bbMat
                bbMx.m41 = (x1 + X2) / 2 + mv.x
                bbMx.m42 = (y1 + y2) / 2 + mv.y
                bbMx.m43 = (z1 + z2) / 2 + mv.z
                .d3dd.SetTransform D3DTS_WORLD, bbMx
                bbMesh.DrawSubset 0
            End If
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If Not modl Is Nothing Then
        ...
        Case vbKeyF12
            ...
            If mBB.Checked Then Set bbMesh = sfCanvas1.d3dx8.CreateBox(sfCanvas1.d3dd, X2 - x1, y2 - y1, z2 - z1, buf)
...
Private Sub Form_Load()
    ...
    mLQ.Checked = CBool(GetSetting("Decapod", App.Title, "LowQuality", "False"))
    ...
    sfCanvas1.screenshotSaveAsJpg = CBool(GetSetting("Decapod", App.Title, "JPG", 0))
   ...
   If strPicView <> "" And booList = False Then
        ' swap dir when double click
        On Error Resume Next
        cdlg.filename = Left(strPicView, InStrRev(strPicView, "\")) & "*.s"
        showFile strPicView
        Screen.MousePointer = vbDefault
    ElseIf strPicView <> "" And booList = True Then
    
        On Error Resume Next
        cdlg.filename = Left(strPicView, InStrRev(strPicView, "\")) & "*.s"
        sfCanvas1.screenshotLocation = strSavePix
        sfCanvas1.setCamera 15#, 2#, 0#, 0#, 2#, 0#, 0, 1, 0
        hShade.value = 245
...
Private Sub loadLang()
    ...
    Case LANG_GERMAN
        lcid = 1024 Or LANG_GERMAN
        If Dir(App.path & "\sview_de.chm") <> "" Then App.HelpFile = App.path & "\sview_de.chm"
        SetThreadLocale lcid
    Case LANG_FRENCH
        lcid = 1024 Or LANG_FRENCH
        If Dir(App.path & "\sview_fr.chm") <> "" Then App.HelpFile = App.path & "\sview_fr.chm"
        SetThreadLocale lcid
    Case LANG_SPANISH
        lcid = 3072 Or (lcid And &HFF)
        If Dir(App.path & "\sview_fr.chm") <> "" Then App.HelpFile = App.path & "\sview_es.chm"
        SetThreadLocale lcid
    End Select
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    SaveSetting "Decapod", App.Title, "LowQuality", mLQ.Checked
    ...
    SaveSetting "Decapod", App.Title, "JPG", sfCanvas1.screenshotSaveAsJpg
...
Private Sub mBB_Click()
    Dim buf As D3DXBuffer
    If modl Is Nothing Then Exit Sub
    mBB.Checked = Not mBB.Checked
    If mBB.Checked Then
        ' get first time only
        If x1 < -9998 Then modl.getBoundingBox x1, y1, z1, X2, y2, z2
        
        lbl = "Width " & Round(X2 - x1, 3) & "m" & vbCr & "Height " & Round(y2 - y1, 3) & "m" & vbCr & "Length " & Round(z2 - z1, 3) & "m"
        Set bbMesh = sfCanvas1.d3dx8.CreateBox(sfCanvas1.d3dd, X2 - x1, y2 - y1, z2 - z1, buf)
    Else
        Set bbMesh = Nothing
        lstLod_Click
    End If
...
Private Sub mLQ_Click()
    mLQ.Checked = Not mLQ.Checked
    Dim i As Long, s As String
    Dim j As Integer
    Dim fmt As String, w As Long, h As Long, loaded As Boolean
    If Not modl Is Nothing Then
        j = modl.nTextures
        modl.force16BitTexture = mLQ.Checked
        For i = 0 To j - 1
            modl.getTexInfo i, s, fmt, w, h, loaded
            
            If loaded Then modl.SetTexture i, modl.loadAce(s)
        Next
    End If
    If Not mod2 Is Nothing Then
        j = mod2.nTextures
        mod2.force16BitTexture = mLQ.Checked
        For i = 0 To j - 1
            mod2.getTexInfo i, s, fmt, w, h, loaded
            If loaded Then mod2.SetTexture i, modl.loadAce(s)
        Next
    End If
End Sub
...
Private Sub mSecond_Click()
    ...
    cdlg.Filter = "MSTS S file|*.s"
    If InStr(cdlg.filename, "\") > 0 Then cdlg.filename = Left(cdlg.filename, InStrRev(cdlg.filename, "\")) & "*.s"
    cdlg.DialogTitle = "Select Shape File"
    On Error Resume Next
    cdlg.ShowOpen
...
Private Sub mSkyFile_Click()
    ...
    If sfCanvas1.isWindowed Then
        ...
        cdlg.Filter = "Image file (bmp,jpg)|*.jpg;*.bmp;*.dds"
...
Private Sub sfCanvas1_MouseMove(Button As Integer, Shift As Integer, x As Single, y As Single)
    ...
    If Shift = vbCtrlMask Then
        If DX <> 0 Then
            mv.z = mv.z + Cos(hAngl) * DX / 300
            mv.x = mv.x - Sin(hAngl) * DX / 300
        End If
        If DY <> 0 Then
            mv.y = mv.y - DY / 300
        End If
...
Private Sub Timer1_Timer()
    ...
    dt = Timer - mTim
    mTim = Timer
    rAng = rAng + dt * hSpeed.value * TORAD * 2
    D3DXMatrixRotationY mt, rAng
    ...
    ' set bounding box matrix and base rotation
    bbMx = mw
    basemx = mw

    If Not mod2 Is Nothing Then
        D3DXMatrixIdentity mw
        DXCopyMemory mw.m41, gmfv, 12
        If gflip Then
            D3DXMatrixRotationY mt, 180 * TORAD + rAng
        Else
            D3DXMatrixRotationY mt, rAng
        End If
        D3DXMatrixMultiply mw, mw, mt
        mw.m41 = mw.m41 + mv.x
        mw.m42 = mw.m42 + mv.y
        mw.m43 = mw.m43 + mv.z
        mod2.setMatrix mw
...
Private Sub initGrid()
    ...
    ' set up the bounding box material
    bbMat.diffuse.a = 0.3
    bbMat.diffuse.R = 1
    bbMat.diffuse.g = 0
    bbMat.diffuse.b = 0
    bbMat.Ambient = bbMat.diffuse
    'bbMat.emissive = bbMat.diffuse
...
Set modx = Nothing
    ...
    If mLQ.Checked Then modx.force16BitTexture = True
    ' load the S file
    modx.filename = s
...
Private Sub saveCam(k As Integer)
    ...
    SaveSetting "Decapod", App.Title, "kcx" & k, mv.x
    SaveSetting "Decapod", App.Title, "kcy" & k, mv.y
    SaveSetting "Decapod", App.Title, "kcz" & k, mv.z
...
Private Sub restCam(k As Integer)
    ...
    mv.x = GetSetting("Decapod", App.Title, "kcx" & k, 0)
    mv.y = GetSetting("Decapod", App.Title, "kcy" & k, 0)
    mv.z = GetSetting("Decapod", App.Title, "kcz" & k, 0)
